...
// ── GET /name-enquiry ───────────────────────────────────────
// Mocks a bank account verification for the Withdraw flow since no active Interswitch endpoints for this are mounted right now.
export async function handleNameEnquiry(req, res) {
    ...
    try {
        ...
        setTimeout(() => {
            const mockName = `STABLEX USER ${accountId.slice(-4)}`;
            console.log(`[CTRL:NameEnquiry] ✅ Resolved to: ${mockName}`);
            res.json({
                success: true,
