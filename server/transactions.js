  ...
  try {
    ...
    if (PLATFORM_FEE_WALLET_ID && transferFee > 0) {
      try {
        ...
        console.error('[FEE ROUTING] Transfer fee credit failed:', feeErr.message);
      }
    }
  ...
  try {
    ...
    if (PLATFORM_FEE_WALLET_ID && fee > 0) {
      try {
        ...
        console.error('[FEE ROUTING] Withdrawal fee credit failed:', feeErr.message);
      }
    }
...
router.post('/swap', protect, async (req, res) => {
  ...
  try {
    ...
    // In production, we find the Treasury user. For demo, we assume a "system" user holds bulk funds.
    const treasuryUser = await User.findOne({ role: 'admin' }).session(session);
    if (!treasuryUser) {
      await session.abortTransaction();
      return res.status(500).json({ success: false, error: 'Treasury account not found. Please contact support.' });
    }
    ...
    if (PLATFORM_FEE_WALLET_ID && estimatedProfit > 0) {
      try {
        ...
        console.error('[FEE ROUTING] Swap spread credit failed:', feeErr.message);
      }
    }
