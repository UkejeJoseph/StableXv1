...
const checkDeposits = async () => {
    try {
        ...
        for (let i = 0; i < wallets.length; i += BATCH_SIZE) {
            ...
            const results = await Promise.allSettled(batch.map(wallet => checkWalletForDeposits(wallet)));

            // Log any failures in the batch specifically
            results.forEach((res, idx) => {
                if (res.status === 'rejected') {
                    console.error(`[POLL] ❌ Batch sub-task failed for wallet ${batch[idx].address}:`, res.reason);
                }
            });
